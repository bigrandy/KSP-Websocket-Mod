...
namespace TestKSP1
{
	...
	public class SettingsNStuff : MonoBehaviour
	{
		...
		void Awake()
		{
            ...
            TargetURL = cfg.GetValue<string>("DeviceTargetURL","ws://192.168.1.77:5000/socket.io/?EIO=3&transport=websocket");
            print("KSPSerialIO: Target URL = " + TargetURL); 
	...
	{
        //Here we'll fork from the SerialPort dependency and instead use a separate websocket handler
        // for outputting the data.  We'll create a new class ConnectionPort and use that to abstract
        public static ConnectionPort Port;
		...
		private void Begin()
		{
            try
            {
                if (null == SettingsNStuff.TargetURL){
                    throw new NotImplementedException("Sorry, unable to initialize a COM port from here!");
                }
                Port = new ConnectionPort(SettingsNStuff.TargetURL, (uint)SettingsNStuff.BaudRate);
                if (Port == null)
                {
                    Debug.Log("KSPSerialIO: Unable to create connection port.");
                }
            } catch (Exception e){
                Debug.Log("KSPSerialIO: Unable to create ConnectionPort for xmitting data:" + e.Message);
                return;
            }
			Port.ReceivedBytesThreshold = 3;
			Port.ReceivedEvent += Port_ReceivedEvent;
            Port.Open();
            Debug.Log("KSPSerialIO: Begin() Completed, new ConnectionPort Created.");
		}
        ...
        */
        ...
        //Invoked by KSP to get things going.
        void Awake()
		{
			if (DisplayFound)
			{
			...
			else
			{
				...
				try
				{
					...
					if (SerialCOMSKey == null)
					{
                        ...
                        if(SettingsNStuff.TargetURL != null && SettingsNStuff.TargetURL.Length > 0){
                            if(Port.IsOpen){
                                Debug.Log("Attempting handshake on websocket url...");
                                doHandshake();
                            }
                        }
					}
					else
					{
						...
						foreach (string PortName in names)
						{
							...
							if (!Port.IsOpen)
							{
								...
								{
									Debug.Log("Error opening conection on port " + Port.PortName + ": " + e.Message);
								}
        ...
        private void Port_ReceivedEvent(object sender, ReceivedDataArgs e)
		{
